...
func TestStoreIndexExists(t *testing.T) {
	...
	db.Set(&schema.KeyValue{Key: []byte(`myFirstElementKey`), Value: []byte(`firstValue`)})
	db.Set(&schema.KeyValue{Key: []byte(`mySecondElementKey`), Value: []byte(`secondValue`)})
	idx1, _ := db.Set(&schema.KeyValue{Key: []byte(`myThirdElementKey`), Value: []byte(`thirdValue`)})
	...
	require.Exactly(t, reference1.GetIndex(), uint64(4))
	require.NotEmptyf(t, reference1, "Should not be empty")

	db.waitForIndexing(reference1.GetIndex())
	...
	require.Exactly(t, reference2.GetIndex(), uint64(5))
	require.NotEmptyf(t, reference2, "Should not be empty")

	db.waitForIndexing(reference2.GetIndex())
	...
	require.Exactly(t, reference3.GetIndex(), uint64(6))
	require.NotEmptyf(t, reference3, "Should not be empty")

	db.waitForIndexing(reference3.GetIndex())
...
func TestStoreIndexEqualKeys(t *testing.T) {
	...
	i1, _ := db.Set(&schema.KeyValue{Key: []byte(`SignerId1`), Value: []byte(`firstValue`)})
	i2, _ := db.Set(&schema.KeyValue{Key: []byte(`SignerId1`), Value: []byte(`secondValue`)})
	i3, _ := db.Set(&schema.KeyValue{Key: []byte(`SignerId2`), Value: []byte(`thirdValue`)})

	zaddOpts1 := &schema.ZAddOptions{
		Set:   []byte(`hashA`),
		Score: &schema.Score{Score: float64(1)},
		Key:   []byte(`SignerId1`),
		Index: &schema.Index{Index: i1.GetIndex()},
	}

	reference1, err1 := db.ZAdd(zaddOpts1)

	require.NoError(t, err1)
	require.Exactly(t, reference1.GetIndex(), uint64(3))
	require.NotEmptyf(t, reference1, "Should not be empty")
	...
	zaddOpts2 := &schema.ZAddOptions{
		...
		Index: &schema.Index{Index: i2.GetIndex()},
	}

	reference2, err2 := db.ZAdd(zaddOpts2)

	require.NoError(t, err2)
	require.Exactly(t, reference2.GetIndex(), uint64(4))
	require.NotEmptyf(t, reference2, "Should not be empty")
	...
	zaddOpts3 := &schema.ZAddOptions{
		...
		Index: &schema.Index{Index: i3.GetIndex()},
	}

	reference3, err3 := db.ZAdd(zaddOpts3)

	require.NoError(t, err3)
	require.Exactly(t, reference3.GetIndex(), uint64(5))
	require.NotEmptyf(t, reference3, "Should not be empty")
...
func TestStore_ZScanPagination(t *testing.T) {
	...
	setName := []byte(`set1`)
	i1, _ := db.Set(&schema.KeyValue{Key: []byte(`key1`), Value: []byte(`val1`)})
	i2, _ := db.Set(&schema.KeyValue{Key: []byte(`key2`), Value: []byte(`val2`)})
	i3, _ := db.Set(&schema.KeyValue{Key: []byte(`key3`), Value: []byte(`val3`)})
	i4, _ := db.Set(&schema.KeyValue{Key: []byte(`key4`), Value: []byte(`val4`)})
	i5, _ := db.Set(&schema.KeyValue{Key: []byte(`key5`), Value: []byte(`val5`)})
	i6, _ := db.Set(&schema.KeyValue{Key: []byte(`key6`), Value: []byte(`val6`)})
	...
	zaddOpts4 := &schema.ZAddOptions{
		...
		Score: &schema.Score{Score: float64(2)},
		Key:   []byte(`key4`),
		Index: &schema.Index{Index: i4.GetIndex()},
	}
	zaddOpts5 := &schema.ZAddOptions{
		Set:   setName,
		Score: &schema.Score{Score: float64(2)},
		Key:   []byte(`key5`),
		Index: &schema.Index{Index: i5.GetIndex()},
	}
	zaddOpts6 := &schema.ZAddOptions{
		Set:   setName,
		Score: &schema.Score{Score: float64(3)},
		Key:   []byte(`key6`),
		Index: &schema.Index{Index: i6.GetIndex()},
	}
...
func TestStore_ZScanOnEqualKeysWithSameScoreAreReturnedOrderedByTS(t *testing.T) {
	...
	idx0, _ := db.Set(&schema.KeyValue{Key: []byte(`key1`), Value: []byte(`val1-C`)})
	db.Set(&schema.KeyValue{Key: []byte(`key2`), Value: []byte(`val2-A`)})
	idx2, _ := db.Set(&schema.KeyValue{Key: []byte(`key1`), Value: []byte(`val1-B`)})
	db.Set(&schema.KeyValue{Key: []byte(`key3`), Value: []byte(`val3-A`)})
	idx4, _ := db.Set(&schema.KeyValue{Key: []byte(`key1`), Value: []byte(`val1-A`)})

	db.ZAdd(&schema.ZAddOptions{
		Set:   []byte(`mySet`),
		Score: &schema.Score{Score: 0},
		Key:   []byte(`key1`),
		Index: &schema.Index{Index: idx2.GetIndex()},
	})
	db.ZAdd(&schema.ZAddOptions{
		Set:   []byte(`mySet`),
		Score: &schema.Score{Score: 0},
	...
	db.ZAdd(&schema.ZAddOptions{
		...
		Score: &schema.Score{Score: 0},
		Key:   []byte(`key1`),
		Index: &schema.Index{Index: idx4.GetIndex()},
	})
	...
	list, err := db.ZScan(zScanOptions)
