...
func TestStoreReference(t *testing.T) {
	...
	root, err := db.Set(&schema.KeyValue{Key: []byte(`firstKey`), Value: []byte(`firstValue`)})

	db.waitForIndexing(root.GetIndex())

	item, err := db.Get(&schema.Key{Key: []byte(`firstKey`)})
	...
	root, err = db.Reference(refOpts)
	require.NoError(t, err)

	db.waitForIndexing(root.GetIndex())

	require.Exactly(t, uint64(2), root.GetIndex())
	require.NotEmptyf(t, root, "Should not be empty")

	firstItemRet, err := db.Get(&schema.Key{Key: []byte(`myTag`)})
