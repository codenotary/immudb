...
type ImmuClientMock struct {
	...
	OpenSessionF          func(ctx context.Context, user []byte, pass []byte, database string) (err error)
	VerifiedGetF          func(context.Context, []byte, ...GetOption) (*schema.Entry, error)
	...
	GetF                  func(context.Context, []byte, ...GetOption) (*schema.Entry, error)
	...
	ExecAllF              func(ctx context.Context, in *schema.ExecAllRequest) (t *schema.TxHeader, err error)
}

// GetOptions ...
func (icm *ImmuClientMock) GetOptions() *Options {
...
// OpenSession ...
func (icm *ImmuClientMock) OpenSession(ctx context.Context, user []byte, pass []byte, database string) error {
	return icm.OpenSessionF(ctx, user, pass, database)
}

// VerifiedGet ...
func (icm *ImmuClientMock) VerifiedGet(ctx context.Context, key []byte, opts ...GetOption) (*schema.Entry, error) {
...
// Get ...
func (icm *ImmuClientMock) Get(ctx context.Context, key []byte, opts ...GetOption) (*schema.Entry, error) {
...
// CreateUser ...
func (icm *ImmuClientMock) ExecAll(ctx context.Context, in *schema.ExecAllRequest) (t *schema.TxHeader, err error) {
	return icm.ExecAllF(ctx, in)
}

func TestConnectionRetriesOpenSessionOnSessionNotFoundError(t *testing.T) {
	totalOpenSessionCalls := 0
	icm := &ImmuClientMock{
		ImmuClient: NewClient(),
		IsConnectedF: func() bool {
			return true
		},
		OpenSessionF: func(ctx context.Context, user []byte, pass []byte, database string) (err error) {
			totalOpenSessionCalls++
			return nil
		},
		ExecAllF: func(ctx context.Context, in *schema.ExecAllRequest) (t *schema.TxHeader, err error) {
	...
	m := NewClientMiddleware(icm)
	if err := m.OpenSession(context.Background(), []byte("fakeUser"), []byte("fakePassword"), "fooDB"); err != nil {
		t.Fatalf("unable to open session in mock, error %v", err)
	}
	_, err := m.ExecAll(context.Background(), &schema.ExecAllRequest{
	...
	if !errors.Is(err, ErrMaxRetries) {
		t.Fatalf("unaexpected error type, got %v", err)
	}

	if expected, got := 6, totalOpenSessionCalls; expected != got {
