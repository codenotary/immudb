...
func TestInMemoryCache(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, uint64(11), root.GetIndex())
	require.Equal(t, []byte{11}, root.GetRoot())

	root, err = imc.Get("server1", "db12")
	require.NoError(t, err)
	require.Equal(t, uint64(12), root.GetIndex())
	require.Equal(t, []byte{12}, root.GetRoot())

	root, err = imc.Get("server2", "db21")
	require.NoError(t, err)
	require.Equal(t, uint64(21), root.GetIndex())
	require.Equal(t, []byte{21}, root.GetRoot())
