...
do
	...
	WLEN=$(expr length $WORKERS)
	BATCHNUM=$((BASE_BATCHNUM/WLEN))
	docker-compose run immudb-tools-kv -addr immudb-async-main -db perf -read-workers 0 -read-batchsize 0 -write-speed 0 -write-workers $WORKERS -write-batchsize $BATCHSIZE -write-batchnum $BATCHNUM -silent -summary 2>&1 | tee -a /tmp/runme.log
	TXS=$(tail -n1 /tmp/runme.log|grep -F "TOTAL WRITE"|grep -Eo '[0-9]+ Txs/s'|cut -d ' ' -f 1)
	STATS+=( $TXS )
	echo "TXS: $TXS, STATS: ${STATS[*]}"
	docker-compose down
...
do
	...
	do
	TXS=${STATS[IDX]}
	echo "$WORKERS	$BATCHSIZE		async	$TXS	$((TXS*BATCHSIZE))"
	CSVLINE="$WORKERS,$BATCHSIZE,async,$TXS,$((TXS*BATCHSIZE))"
	CSV_LINES+=( $CSVLINE )
	IDX=$((IDX+1))
	done
done
