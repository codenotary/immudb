...
func (cl *commandline) configureFlags(cmd *cobra.Command) error {
	...
	cmd.PersistentFlags().Bool("audit-verify-values", false, "When set, audit-mode also fetches each KV entry committed since the last audited state and verifies that the on-disk value matches its committed hash. Detects on-disk value tampering / bit-rot at the cost of O(entries) extra reads per cycle. Non-KV entries (SQL, document, system) are skipped.")
	...
	viper.BindPFlag("audit-verify-values", cmd.PersistentFlags().Lookup("audit-verify-values"))
