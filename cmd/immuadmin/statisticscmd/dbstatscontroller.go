...
func (p *dbStatsController) Resize() {
	p.resize()
	ui.Render(p.Grid)
}
...
func (p *dbStatsController) Render(data *map[string]*dto.MetricFamily) {
...
func (p *dbStatsController) initUI() {
	p.resize()
...
func newDBStatsController() Controller {
	w, _, _ := terminal.GetSize(0)
	if w <= 0 {
		w = terminalWidth
	}
	ctl := &dbStatsController{
		Grid: ui.NewGrid(),
