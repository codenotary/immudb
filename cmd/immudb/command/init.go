...
func (cl *Commandline) setupFlags(cmd *cobra.Command, options *server.Options) {
	...
	cmd.Flags().Duration("max-session-inactivity-time", 1*time.Hour, "max session inactivity time is a duration after which an active session is declared inactive by the server. A session is kept active if server is still receiving requests from client (keep-alive or other methods)")
...
func setupDefaults(options *server.Options) {
	...
	viper.SetDefault("max-session-inactivity-time", 1*time.Hour)
