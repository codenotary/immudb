...
func (cl *Commandline) setupFlags(cmd *cobra.Command, options *server.Options) {
	...
	cmd.Flags().Bool("replica", false, "set systemdb and defaultdb as replica")
	cmd.Flags().String("master-address", "", "master address (if replica=true)")
	cmd.Flags().Int("master-port", 3322, "master port (if replica=true)")
	cmd.Flags().String("replica-username", "", "username used for replication of systemdb and defaultdb")
	cmd.Flags().String("replica-password", "", "password used for replication of systemdb and defaultdb")
