...
curl  -X POST -H "authorization: Bearer $TOKEN" -d "{\"sql\": \"$SQL\"}" http://localhost:9997/v1/immurestproxy/db/sqlexec

export SQL="UPSERT INTO table1(id, title) VALUES (1, 'Fancy Title');"
curl  -X POST -H "authorization: Bearer $TOKEN" -d "{\"sql\": \"$SQL\"}" http://localhost:9997/v1/immurestproxy/db/sqlexec

export SQL="SELECT id,title FROM table1;"
curl  -X POST -H "authorization: Bearer $TOKEN" -d "{\"sql\": \"$SQL\"}" http://localhost:9997/v1/immurestproxy/db/sqlquery
