...
func TestQueryDocument(t *testing.T) {
	...
	// create collection
	collectionName := "mycollection"
	err := engine.CreateCollection(ctx, collectionName, map[string]sql.SQLValueType{
		"pincode": sql.IntegerType,
		"country": sql.VarcharType,
		"idx":     sql.IntegerType,
	})
	...
	for i := 1.0; i <= 10; i++ {
		_, _, err = engine.CreateDocument(ctx, collectionName, &structpb.Struct{
			Fields: map[string]*structpb.Value{
				"pincode": {
...
func TestDocumentUpdate(t *testing.T) {
	...
	// add document to collection
	docID, _, err := engine.CreateDocument(ctx, collectionName, &structpb.Struct{
		Fields: map[string]*structpb.Value{
			"country": {
				Kind: &structpb.Value_StringValue{StringValue: "wonderland"},
			},
	...
	_, revision, err := engine.UpdateDocument(ctx, collectionName, docID, &structpb.Struct{
		Fields: map[string]*structpb.Value{
			"country": {
				Kind: &structpb.Value_StringValue{StringValue: "wonderland"},
			},
