...
// NewJSONFileLogger returns a json logger which outputs to file
func NewJSONFileLogger(name string, file string) (logger Logger, out *os.File, err error) {
	...
	return &JsonLogger{
		...
		level:      int32(logLevelFromEnvironment()),
	}, out, nil
}

// NewJSONLogger returns a json logger.
func NewJSONLogger(opts *Options) *JsonLogger {
	if opts == nil {
		opts = &Options{}
	}
...
func (w *writer) Flush(level LogLevel) (err error) {
