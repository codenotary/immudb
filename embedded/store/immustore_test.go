...
// TestTxbsPoolEvictionSafety exercises the critical safety contract
// for the txbs buffer pool: after a cached tx is evicted and its
// backing slice returns to the pool, subsequent readers that had
// already obtained the slice (via appendableReaderForTx → slicedReaderAt)
// must still see correct bytes. A tiny TxLogCacheSize forces eviction
// on every commit; running under `go test -race` is what catches any
// use-after-free if the copy-on-Get path ever regresses.
func TestTxbsPoolEvictionSafety(t *testing.T) {
	immuStore, err := Open(t.TempDir(),
		DefaultOptions().WithSynced(false).WithTxLogCacheSize(2))
	require.NoError(t, err)
	defer immuStore.Close()

	// Write a handful of txs; cache size 2 means txs 1-3 have all
	// been evicted by the time we commit tx 5+.
	const nTxs = 10
	txIDs := make([]uint64, nTxs)
	for i := 0; i < nTxs; i++ {
		tx, err := immuStore.NewWriteOnlyTx(context.Background())
		require.NoError(t, err)
		require.NoError(t, tx.Set([]byte(fmt.Sprintf("k-%d", i)), nil, []byte(fmt.Sprintf("v-%d", i))))
		hdr, err := tx.Commit(context.Background())
		require.NoError(t, err)
		txIDs[i] = hdr.ID
	}

	// Read every tx back. The early ones went through the cache-miss
	// path; the late ones hit the cache before/after eviction. Under
	// -race, any aliasing between an in-flight slicedReaderAt and a
	// pool-recycled buffer shows up here.
	for _, id := range txIDs {
		tx := NewTx(immuStore.MaxTxEntries(), immuStore.MaxKeyLen())
		require.NoError(t, immuStore.ReadTx(id, false, tx))
	}
}
...
// between baseline and patch localise the pooling win.
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			...
			for j := 0; j < batchSize; j++ {
				k := make([]byte, keyLen)
				binary.BigEndian.PutUint64(k, uint64(rnd.Int63()))
				v := make([]byte, valLen)
				binary.BigEndian.PutUint64(v, uint64(j))
