...
Embed [immudb](https://github.com/codenotary/immudb) in Node.js — **in-process, via WASM + WASI**. An append-only, cryptographically verifiable **KV + SQL** store with **no server, no container, and no native prebuilds**: a single portable `immudb.wasm` that runs anywhere Node 20+ runs (macOS/Linux/Windows, any arch).
...
Requires **Go 1.24+** (for `//go:wasmexport`). The npm package ships a prebuilt `node/immudb.wasm`; to rebuild:
